...
void pt_kern_vmmap() {
    int ret = pt_map_pages(kernel_pagetable, TRAMPOLINE, (uint64) trampoline, PGSIZE, PTE_R | PTE_X);
    ASSERT_EQ(ret,0,"trampoline map");

    ret = pt_map_pages(kernel_pagetable, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    uint64 idx = UART0;
    ...
    ret = pt_map_pages(kernel_pagetable, (uint64) endTextSect, (uint64) endTextSect, PHYSTOP - (uint64) endTextSect,
                       PTE_R | PTE_W);
